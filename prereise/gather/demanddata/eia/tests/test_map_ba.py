import pandas as pd
from prereise.gather.demanddata.eia.map_ba import map_to_loadzone, \
...
def test_loadzone_mapping_case():
    ...
    assert(zone_demand.values.T.tolist() == [[(1/4)+(2/3)]*3,
                                             [(3/4)+(4/3)+3]*3])

def test_loadzone_mapping_has_equal_total_demand():
    bus_map, agg_demand = create_loadzone_dataframe()
    zone_demand = map_to_loadzone(agg_demand, bus_map)
    assert_series_equal(agg_demand.sum(axis=1), zone_demand.sum(axis=1),
                        check_dtype=False)
...
def create_loadzone_dataframe():
    bus_map_data = {'BA': ['A','A','B','A','B','C'],
                    'zone_name': ['X','X','X','Y','Y','Y'], 'Pd': range(0,6)}
...
def create_ba_to_region_dataframe():
    start_data = {'A': range(0,10), 'B': range(10,20), 'C': range(20,30),
                  'D': range(30,40), 'E': range(40,50)}
