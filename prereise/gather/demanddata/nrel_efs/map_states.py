...
def map_to_loadzone(agg_dem, save=None):
    ...
    :param pandas.DataFrame agg_dem: DataFrame of the aggregated sectoral demand data,
        where the rows are time steps and the columns are the states.
    ...
    if agg_dem.index.name != "UTC Time":
    ...
    abv2id = {}
    for x in id2abv:
        if id2abv[x] not in abv2id:
            abv2id[id2abv[x]] = {x}
        else:
            abv2id[id2abv[x]].add(x)
    ...
    agg_dem_lz = pd.DataFrame(index=agg_dem.index, columns=list(id2abv.keys()))
    for x in id2abv:
        agg_dem_lz[x] = agg_dem[id2abv[x]] * pd_frac[id2abv[x]][x]
