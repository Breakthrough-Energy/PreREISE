...
def get_cluster_ts(df, start, end, ts_length):
    ...
    temp = df_norm.loc[
        (df_norm.index.hour >= start) & (df_norm.index.hour <= end)
    ].copy()
...
def similar_day(
    df, user, weather, idx, y_pred, cluster_num, title, ts_length, mean=True
):
    ...
    if len(cluster) != 0:
        ...
        # cluster_weather = weather.loc[
        #     weather.index.intersection(cluster_weather_idx)
        # ].copy()
        ...
        user_similar_day = (
            df[user].loc[df.index.intersection(cluster_weather_idx)].copy()
        )

        period = (
            user_similar_day.index.to_series()
        ...
        for i in period:
            ...
            if (
                user_similar_day.loc[
                    (user_similar_day.index >= start) & (user_similar_day.index < end)
                ].mean()
                != 0
            ) and (
                len(
                    user_similar_day.loc[
                        (user_similar_day.index >= start)
                        & (user_similar_day.index < end)
                    ]
                )
                == 24
            ):
                new_day = user_similar_day.loc[
                    (user_similar_day.index >= start) & (user_similar_day.index < end)
                ]
...
def train(weather, cluster_num, season, dayofweek, time, ts_length):
    if dayofweek == "Weekday":
        weather_temp = weather.loc[
            (weather["day of week"] != "Saturday")
            & (weather["day of week"] != "Sunday")
        ]
    ...
    if season == "Summer":
        weather_temp = weather_temp.loc[
            (weather_temp.index.month >= 5) & (weather_temp.index.month <= 10)
        ]
    elif season == "Winter":
        weather_temp = weather_temp.loc[
            (weather_temp.index.month < 5) | (weather_temp.index.month > 10)
...
def plot_flex_label(df, Y, arr, plot_profile=True, plot_ratio=True):  # noqa: N803
    if len(arr) == 1:
        size = 1
    ...
    if plot_ratio:
        ...
        ratio = np.array(
            [
                round((high / temp_df.shape[1]) * 100, 2),
                round((med / temp_df.shape[1]) * 100, 2),
                round((low / temp_df.shape[1]) * 100, 2),
            ]
        )
        ...
        plt.pie(
            ratio,
            labels=[f"high: {ratio[0]}%", f"medium: {ratio[1]}%", f"low: {ratio[2]}%"],
        )
        plt.legend()
...
def get_gen_flex_label(generated, seq_num, arr, plot=True):
    ...
    for i in generated.columns:
        ...
        other_avg = generated[i].loc[np.r_[0:14, 20:-1]].mean()
    ...
    if plot:
        ...
        ratio = np.array(
            [
                round((high / seq_num) * 100, 2),
                round((med / seq_num) * 100, 2),
                round((low / seq_num) * 100, 2),
            ]
        )
...
def get_x_and_y(df, norm=True, standard=False):
    X = df.iloc[:-3, :].copy()  # noqa: N806
    Y = df.iloc[-3:, :].copy()  # noqa: N806

    if norm:
        x_max = X.max()
        x_min = X.min()

        X = (X - x_min) / (x_max - x_min)  # noqa: N806

    if standard:
        x_max = X.mean()
        x_min = X.std()
        X = (X - x_max) / x_min  # noqa: N806

    else:
        x_max = None
        x_min = None

    X = X.values.T  # noqa: N806
    Y = Y.values.T  # noqa: N806

    return X, Y, x_max, x_min  # noqa: N806
