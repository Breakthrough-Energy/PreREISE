...
from prereise.gather.translate_zones import filter_interesting_zones, format_zone_df, translate_zone_set


def test_filter_interesting_zones():
    df = pd.DataFrame({ 'a': [0, 0, 1, 0.5432], 'b': [0.9, 1, 0, 0.5568], 'c': [0, 0, 0, 0], 'd': [0.1, 0, 0, 0]}, index=['w', 'x', 'y', 'z'])
...
def test_format_zone_df_invalid_geometry():
    invalid_bowtie = Polygon([(-34.58, -58.66), (-15.78, -47.91), (-33.45, -70.66), (4.60, -74.08)])
...
def test_translate_zone_set_happy_path():
    ...
    zones_2 = [ Polygon([(-34.58, -58.66), (-15.78, -47.91), (4.60, -74.08), (-33.45, -70.66)]) ]
    mock_gdf = gpd.GeoDataFrame({ 'geometry': zones_1 }, index=['a', 'b'], crs='EPSG:4269')
    mock_gdf_2 = gpd.GeoDataFrame({ 'geometry': zones_2 }, index=['c'], crs='EPSG:4269')

    assert translate_zone_set(mock_gdf, mock_gdf_2).to_dict() == {'c': {'a': 1.0, 'b': 1.0}}
    assert round(translate_zone_set(mock_gdf_2, mock_gdf), 3).to_dict() == {'a': {'c': 0.22}, 'b': {'c': 0.78}}
...
def test_translate_zone_set_self_overlap():
    ...
    assert translate_zone_set(mock_gdf, mock_gdf_2).to_dict() == {'c': {'a': 1.0, 'b': 1.0}}
    assert round(translate_zone_set(mock_gdf_2, mock_gdf), 3).to_dict() == {'a': {'c': 0.211}, 'b': {'c': 0.789}}
...
def test_translate_zone_set_gap():
    ...
    assert translate_zone_set(mock_gdf, mock_gdf_2).to_dict() == {'c': {'a': 1.0, 'b': 1.0}}
    assert round(translate_zone_set(mock_gdf_2, mock_gdf), 3).to_dict() == {'a': {'c': 0.238}, 'b': {'c': 0.762}}
...
def test_translate_zone_set_isolated():
    ...
    zones_2 = [ Polygon([(-34.58, -58.66), (-15.78, -47.91), (4.60, -74.08), (-33.45, -70.66)]) ]
    mock_gdf = gpd.GeoDataFrame({ 'geometry': zones_1 }, index=['a', 'b', 'c'], crs='EPSG:4269')
    mock_gdf_2 = gpd.GeoDataFrame({ 'geometry': zones_2 }, index=['d'], crs='EPSG:4269')

    mock_gdf.plot(color='green', alpha=0.5);

    assert translate_zone_set(mock_gdf, mock_gdf_2).to_dict() == {'d': {'a': 1.0, 'b': 1.0, 'c': 0.0}}
    assert round(translate_zone_set(mock_gdf_2, mock_gdf), 3).to_dict() == {'a': {'d': 0.22}, 'b': {'d': 0.78}, 'c': {'d': 0}}
