...
def get_hifld_electric_substations(path):
    ...
    """
    data = pd.read_csv(path)
    return data.query("STATUS == 'IN SERVICE' and STATE in @abv2state and LINES != 0")


def get_hifld_electric_power_transmission_lines(path):
    """Read the HIFLD Electric Power Transmission Lines CSV file and keep in service
    lines.

    :param str path: path to file. Either local or URL.
    :return: (*pandas.DataFrame*) -- in service electric power transmission lines.
    """
    data = pd.read_csv(path)
    return data.query("STATUS == 'IN SERVICE'")


def get_hifld_electric_power_transmission_lines_topology(path):
    ...
    properties = pd.DataFrame([line["properties"] for line in data["features"]])
