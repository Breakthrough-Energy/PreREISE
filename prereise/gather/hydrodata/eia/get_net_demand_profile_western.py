...
def get_net_demand_profile_western(state, s):
    ...
    if state not in state_name:
        print(
            "%s is incorrect. Possible states are: %s"
    ...
    wind_plant_id_list = list(
        plant[(plant.type == "wind") & (plant.zone_name.isin(state_name[state]))].index
    )
    solar_plant_id_list = list(
        plant[(plant.type == "solar") & (plant.zone_name.isin(state_name[state]))].index
    )
