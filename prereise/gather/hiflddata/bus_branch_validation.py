...
def get_across_branch(branch,bus_dict):
    ...
    for br in branch.iloc:
        if(bus_dict[br['from_bus_id']] != bus_dict[br['to_bus_id']]):
            if(bus_dict[br['from_bus_id']] == 'Texas' or bus_dict[br['to_bus_id']] == 'Texas'):
...
def get_update_branch_bus(branch_need_update,bus_acc):
    bus_will_update={}
    branch_will_update={}
    need_to_jus={}
    br_delete=[]
...
def bus_branch_validation():
    ...
    bus_will_update, branch_will_update, need_to_jus, br_delete, bus_delete = get_update_branch_bus(branch_need_update,bus_acc)
