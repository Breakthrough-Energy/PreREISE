...
def new_plant(plants,pl_curve, bus_line_total_capa,bus_load, ziplist, ZipOfsub_dict, avaiable_load,add_plant,plant_remove,new_plant_id):
    ...
    for plant in plants.iloc:
        ...
        if((plant['Pmax'] > 1500) or bus_capacity < 1.2*(plant['Pmax'] - true_load)):
            ...
            nei_bus, avaiable_load = find_near_sub(lat, lon, pmax, re, Zip, ziplist, ZipOfsub_dict, avaiable_load)
            for bu in nei_bus:
                add_plant[new_plant_id] = [pmax*0.2, pmin*0.2, bu, re, Type, pl_curve[plant['plant_id']][0], pl_curve[plant['plant_id']][1],pl_curve[plant['plant_id']][2]]
...
if __name__ == "__main__":
    ...
    add_plant,plant_remove = new_plant(plants,pl_curve, bus_line_total_capa,bus_load, ziplist, ZipOfsub_dict, avaiable_load,add_plant,plant_remove,new_plant_id)

    for pl in add_plant:
    #a = [[bu,1,0,0,0,0,zone_id,0,0,0,0,0,0,0,add_bus[bu][3],add_bus[bu][4]]]
        new = pd.DataFrame({'plant_id':pl,
