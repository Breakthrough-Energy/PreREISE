...
def Plant_agg(clean_data, ZipOfsub_dict, loc_of_plant, LocOfsub_dict, Pmin, region, points):
    ...
    plant_dict = {}
    sto = ['HYDROELECTRIC PUMPED STORAGE', 'BATTERIES', 'NATURAL GAS WITH COMPRESSED AIR STORAGE',
           'SOLAR THERMAL WITH ENERGY STORAGE', 'FLYWHEELS']
...
def write_gen(plant_dict, type_dict, curve):
    ...
    with open("output/gencost.csv", "w", newline="") as gencost:
        ...
        for key in plant_dict:
            ...
            if (pid in curve):
                csv_writer.writerow(
                    [key[0] + '-' + key[1] + '-' + key[2], type_d[plant_dict[key][7]], 1, curve[pid][0], curve[pid][1],
                     curve[pid][2]]
                )
            else:
                print(pid)
