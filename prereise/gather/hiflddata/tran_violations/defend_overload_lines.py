...
if __name__ == "__main__":
    all_exce = get_exce()
    violation = pd.DataFrame(columns = ['branchid','trans_viol','branch_rating','percentage'])
    for csv in all_exce:
        df1 = pd.read_csv(csv)
    ...
    for br in branches.iloc:
        if br['branch_id'] in br_vio:
            ...
            add_branch[new_branch_id] = [br['from_bus_id'], br['to_bus_id'], br['rateA']*num, br['interconnect'],br['x']/num]
            new_branch_id = new_branch_id + 1

    for br in add_branch:
        new = pd.DataFrame({'branch_id':br,
