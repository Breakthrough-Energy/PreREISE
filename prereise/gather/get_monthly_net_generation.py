...
def get_monthly_net_generation(state, filename, fuel_type, trim_eia_form_923):
    ...
    if state not in state_name:
        print("%s is incorrect. Possible states are: %s" % (state, state_name))
    ...
    if fuel_type not in all_fuel_type:
        print(
            "%s is incorrect. Possible fuel types are: %s"
    ...
    net_generation_by_plant = plant_generation[
        (plant_generation["Plant State"] == state)
        & (plant_generation["AER\nFuel Type Code"].isin(all_fuel_type[fuel_type]))
    ].copy()
